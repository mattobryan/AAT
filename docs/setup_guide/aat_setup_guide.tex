...
\begin{tikzpicture}[node distance=1.1cm and 1.6cm, font=\small,
  ...
  \node[box, below=1.6cm of hf] (rt) {Claude routine\\every 30 min\\\code{scripts/status.py}};
...
\code{pretr\_Linf} (fine-tuning start)             & 83.7 & 48.1 & 59.8 & 7.7 & 38.5 & sanity \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{enumerate}
  ...
  \item Sanity cell: evaluates \code{pretr\_Linf}. Expect $\approx$83.7 / 48.1 / 59.8 / 7.7 / 38.5.
...
The routine runs every 30 minutes in a fresh cloud session. It checks out the branch, runs
...
Sanity numbers far from 83.7 / 38.5 & evaluation or data problem: stop and investigate before training. \\
